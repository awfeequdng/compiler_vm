...
int main() {
    ...
    definition.addDefinition(line);
    definition.addDefinition("nihao", "world");
    definition.printDefinitions();
    Result r;
    auto def = definition.getDefinition("nihao", r);
    if (r.isFailed()) {
    ...
    def = definition.getDefinition("test", r);
    if (r.isFailed()) {
        std::cout << colorizer::colorize(r.messages()[0].message(), term_colors_t::red) << std::endl;
    } else {
